...
# 1. Configuración de API (Habka Ammaanka ah)
# Sparked Host 'Variables' ku dhex qor BYBIT_API_KEY iyo BYBIT_API_SECRET
API_KEY = os.getenv('BYBIT_API_KEY')
API_SECRET = os.getenv('BYBIT_API_SECRET')

if not API_KEY or not API_SECRET:
    print("CILAD: API Keys-ka lagama helin Sparked Host Variables. Fadlan geli!")
    sys.exit()
            ...
            with lock:
                if len(active_trades) < MAX_SLOTS:
                    ...
                    if resp.get('retCode') != 0:
                        print(f"CILAD API: {resp.get('retMsg')}")
                        time.sleep(60)
                        continue
                    ...
                    for symbol, change, price in candidates:
                        if symbol not in active_trades and get_rsi(symbol) < 70:
                            ...
                            if order.get('retCode') == 0:
                                print(f"BUY SUCCESS: {symbol} at {price}")
