...
# 1. Configuración de API (Cusub)
API_KEY = "OlqpzaYpNYC8lT8cDF"
API_SECRET = "9Frgl1VnuYllIx2q2Cz85ZBcIByIz5RPkkrj" 
                    ...
                    for symbol, change, price in candidates:
                        if symbol not in active_trades and get_rsi(symbol) < 70:
                            ...
                            if order.get('retCode') == 0:
                                ...
                                Thread(target=monitor_trade, args=(symbol, price, str(qty)), daemon=True).start()
                                break
            time.sleep(30)
        except Exception as e:
