...
# FADLAN: Keys-kaaga halkan geli markaad server-ka geyneyso uun
API_KEY = "1aw6be1DbXqkVMX3ok"
API_SECRET = "GELI_SECRET_KAAGA_HADA" 
...
def get_qty_precision(step):
    step_str = str(step)
    if '.' in step_str:
        return len(step_str.split('.').rstrip('0'))
    return 0

# 4. Monitoring Logic (Safey oo la habeeyey)
def monitor_trade(symbol, entry_price, qty):
    highest_price = entry_price
    start_time = time.time()  # Tani hadda way shaqaynaysaa
    sl_price = entry_price * 0.98
    is_breakeven = False
    
    print(f"LOG: Bilaabaya ilaalinta {symbol} | Entry: {entry_price}")
    
    while True:
        time.sleep(5)
        try:
            ticker = session.get_tickers(category="spot", symbol=symbol)
            curr_price = float(ticker['result']['list']['lastPrice'])
            ...
            if not is_breakeven and curr_price >= entry_price * BREAKEVEN_TRIGGER:
                ...
                is_breakeven = True
                print(f"ACTION: {symbol} Stop Loss -> Breakeven")
            ...
            if highest_price >= entry_price * TRAILING_START:
                if curr_price <= highest_price * (1 - TRAILING_DROP):
                    ...
                    print(f"WIN: {symbol} - Trailing Profit Closed!")
                    break
            
            if curr_price <= sl_price:
                session.place_order(category="spot", symbol=symbol, side="Sell", orderType="Market", qty=qty)
                print(f"EXIT: {symbol} - Stop Loss Hit.")
                break
                
            # Time Limit Logic (Ciladii halkan waa laga saaray)
            if time.time() - start_time > TIME_LIMIT_SEC:
                session.place_order(category="spot", symbol=symbol, side="Sell", orderType="Market", qty=qty)
                print(f"TIME: {symbol} - 1 Saac ayaa dhammaaday, waa la iibiyey.")
                break
        except Exception as e:
                ...
                if len(active_trades) < MAX_SLOTS:
                    ...
                    for symbol, change, price in candidates:
                        if symbol not in active_trades and get_rsi(symbol) < 70:
                            ...
                            lot = instr['result']['list']['lotSizeFilter']
                            step = float(lot['basePrecision'] or lot['qtyStep'])
                            decimals = get_qty_precision(step)
                            qty = round((TRADE_SIZE / price), decimals)
                            
                            order = session.place_order(category="spot", symbol=symbol, side="Buy", orderType="Market", qty=qty)
                            if order.get('retCode') == 0:
                                print(f"BUY: {symbol} | Price: {price}")
                                with lock:
