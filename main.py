...
def get_qty_precision(step):
    step_str = str(step)
    if '.' in step_str: return len(step_str.split('.')[1].rstrip('0'))
    return 0

# 4. Monitoring Logic (Iibinta)
def monitor_trade(symbol, entry_price, qty):
    highest_price = entry_price
    start_time = time.time()
    sl_price = entry_price * 0.98
    is_breakeven = False
    print(f"LOG: Ilaalinta {symbol} ayaa bilaabatay.")
    while True:
        time.sleep(5)
        try:
            ...
            if highest_price >= entry_price * TRAILING_START:
                if curr_price <= highest_price * (1 - TRAILING_DROP):
                    ...
                    print(f"WIN: {symbol} Trailing Profit!")
                    break
