...
session = HTTP(testnet=False, api_key=BYBIT_API_KEY, api_secret=BYBIT_API_SECRET, recv_window=10000)
...
SKIP_LARGE_CAPS = ['BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','XRPUSDT','ADAUSDT','DOTUSDT','LINKUSDT','MATICUSDT','DOGEUSDT']
                    ...
                    for coin in account.get('coin', []):
                        if coin.get('coin') == 'USDT':
                            ...
                            if bal > 0:
                                last_known_balance = bal
                                return bal
            elif 'totalEquity' in result:
                bal = float(result['totalEquity'])
                if bal > 0:
                    last_known_balance = bal
                    return bal
...
def get_5m_momentum(symbol):
    try:
        ...
        now, prev = float(data[0][4]), float(data[1][4])
...
def get_precision_qty(symbol, amount_usd):
    try:
        ...
        qty = round(qty, 8)
        if qty <= 0 or qty*price < 10.0: return None,None
        return qty, price
    except:
        return None,None
...
def buy(symbol, amount_usd):
    ...
    try:
        ...
        if not qty: return False, None, None
...
def sell(symbol, qty_to_sell):
    try:
        ...
        order = session.place_order(category="spot", symbol=symbol, side="Sell", orderType="Market", qty=qty_str)
...
def monitor_trade(symbol):
    ...
    entry, qty, start_time = trade['entry'], trade['qty'], trade['time']
    highest = entry
    while True:
        time.sleep(5)
        with lock:
            if symbol not in active_trades: break
        try:
            ticker = session.get_tickers(category="spot", symbol=symbol)
            curr_price = float(ticker['result']['list'][0]['lastPrice'])
            change = (curr_price/entry)-1
            if change >= (TAKE_PROFIT_PCT-1):
                sell_full_position(symbol, qty, entry); break
            if curr_price > highest: highest = curr_price
            if curr_price <= highest * TRAIL_SL_PCT:
                sell_full_position(symbol, qty, entry); break
...
def get_top_candidates():
    ...
    try:
        ...
        for t in tickers:
            ...
            if sym in SKIP_LARGE_CAPS or 'UP' in sym or 'DOWN' in sym: continue
            vol = get_24h_volume_usd(sym)
            if vol < MIN_VOLUME_USD or vol > MAX_VOLUME_USD: continue
            temp.append((sym, vol))
        temp.sort(key=lambda x: x[1], reverse=True)
        temp = temp[:20]
        for sym, vol in temp:
            mom = get_5m_momentum(sym)
            if mom < MIN_MOMENTUM: continue
            rsi = get_rsi(sym)
            if rsi < 20 or rsi > 95: continue
...
def engine():
    ...
    while is_running:
        ...
        if today != trade_date:
            ...
            trade_date = today
        with lock:
            if pause_until > 0 and time.time() < pause_until: continue
            elif pause_until > 0 and time.time() >= pause_until: pause_until = 0
        ...
        if get_btc_change() < BTC_MIN_CHANGE: continue
        ...
        for sym, mom, vol in candidates:
            ...
            if trade_size < MIN_TRADE_USD: continue
