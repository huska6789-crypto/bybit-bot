...
# 1. Configuración de API (Geli keys-kaaga)
...
TRADE_SIZE = 15
MAX_SLOTS = 1
TRAILING_START = 1.015     # Wuxuu bilaabayaa 1.5% markay gaarto
TRAILING_DROP = 0.003      # 0.3% dib u dhac haddii uu jiro iibi
BREAKEVEN_TRIGGER = 1.005  # 0.5% ka dhig Breakeven
TIME_LIMIT_SEC = 3600 

active_trades = {}
lock = Lock()

# 3. RSI Calculation (SAXNAAN)
def get_rsi(symbol):
    try:
        ...
        # Khadkan hoose ayaa hore u khaldanaa, hadda waa sax:
        closes = [float(k[4]) for k in data] # Bybit kline index 4 waa 'Close'
        closes.reverse()
...
def monitor_trade(symbol, entry_price, qty):
    ...
    while True:
        ...
        try:
            ...
            curr_price = float(ticker['result']['list'][0]['lastPrice']) # Saxid halkan ah
        ...
        try:
            with lock:
                if len(active_trades) < MAX_SLOTS:
                    ...
                    candidates.sort(key=lambda x: x[1], reverse=True) # Saxid halkan ah
                    ...
                    for symbol, change, price in candidates:
                        if symbol not in active_trades and get_rsi(symbol) < 70:
                            ...
                            lot = instr['result']['list'][0]['lotSizeFilter'] # Saxid halkan ah
